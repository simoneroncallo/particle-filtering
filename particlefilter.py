# This module contains the particle filter class, as well as the state and the
# observation models.

import numpy as np
import scipy as sp

def state_model(x1,x2,dx):
...
def likelihood(yObs, pct, rng):
    ...
    return condPdf(yObs, observation_model(pct.x1,pct.x2,rng), [[1,0],[0,1]]) 
...
class Particle:
    ...
    def move(self, rng, keep = True):
        x1Upd, x2Upd = state_model(self.x1, self.x2, self.speed)
        if keep == True:
    ...
    def observe(self, rng):
        return observation_model(self.x1, self.x2, rng)